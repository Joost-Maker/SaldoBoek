...
## PO amendment 2: no IBAN masking, the model gets the full transaction (Joost, 2026-09-18)

"Waarom al deze moeite. Het is een lokaal model. We kunnen gewoon de hele regel in het LLM gooien." The FR "No IBANs in the prompt" and **AC13 are withdrawn**. That requirement was added during refinement, not asked for by the PO, and it guards against nothing: the model runs on `127.0.0.1` only, and the server doesn't log prompts (checked: no verbose/log flags in the unit, 0 prompt lines in the journal). The model now gets per sample `datum · bedrag · omschrijving` in full (whitespace collapsed, capped at 1000 chars). Unchanged: loopback-only network, no data on stdout, read-only DB, the GPU guard.
